...
* В качестве узлов хранения допустимо использовать файлы на диске (по одному файлу на реплику/узел).
...
| Метод                                      | Поведение                                                                                        |
|--------------------------------------------|--------------------------------------------------------------------------------------------------|
| `GET /key?ack=X`                           | Возвращать данные, если подтвердили ответ ≥ `ack` реплик. Если доступно меньше – возвращать 50x. |
| `PUT /key?ack=X` <br/> `DELETE /key?ack=X` | Ждать подтверждений от ≥ `ack` реплик. При недоступности нужного числа – возвращать 50x.         |
