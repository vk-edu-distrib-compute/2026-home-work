...
public class JokerygaKVService implements KVService {
    private static final Logger log = LoggerFactory.getLogger(JokerygaKVService.class);
    ...
    public JokerygaKVService(int port, Dao<byte[]> dao) {
        this.port = port;
        this.dao = dao;
    }
    ...
    private static final class StatusHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            if (!METHOD_GET.equals(exchange.getRequestMethod())) {
                sendTextResponse(exchange, HttpStatus.METHOD_NOT_ALLOWED, METHOD_NOT_ALLOWED_MESSAGE);
                return;
            }

            sendTextResponse(exchange, HttpStatus.OK, OK_MESSAGE);
        }
    ...
    private final class EntityHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            if (id == null) {
                sendTextResponse(exchange, HttpStatus.BAD_REQUEST, MISSING_ID_MESSAGE);
                return;
            }
            ...
            try {
                ...
                sendTextResponse(exchange, HttpStatus.BAD_REQUEST, e.getMessage());
            } catch (NoSuchElementException e) {
                sendTextResponse(exchange, HttpStatus.NOT_FOUND, e.getMessage());
            } catch (IOException e) {
                log.error("IO error handling request", e);
                sendTextResponse(exchange, HttpStatus.INTERNAL_ERROR, INTERNAL_ERROR_MESSAGE);
            }
        }
        ...
        private void processRequest(HttpExchange exchange, String id) throws IOException {
            ...
            switch (method) {
                ...
                case null, default -> sendTextResponse(
                    exchange, HttpStatus.METHOD_NOT_ALLOWED, METHOD_NOT_ALLOWED_MESSAGE);
            }
        }
        ...
        private void handleGet(HttpExchange exchange, String id) throws IOException {
            ...
            sendBinaryResponse(exchange, HttpStatus.OK, data);
        }

        private void handlePut(HttpExchange exchange, String id) throws IOException {
            byte[] data = exchange.getRequestBody().readAllBytes();
            dao.upsert(id, data);
            sendBinaryResponse(exchange, HttpStatus.CREATED, CREATED_MESSAGE.getBytes(StandardCharsets.UTF_8));
        }

        private void handleDelete(HttpExchange exchange, String id) throws IOException {
            dao.delete(id);
            sendBinaryResponse(exchange, HttpStatus.ACCEPTED, DELETED_MESSAGE.getBytes(StandardCharsets.UTF_8));
        }

        private void sendTextResponse(HttpExchange exchange, int statusCode, String message) throws IOException {
            sendBinaryResponse(exchange, statusCode, message.getBytes(StandardCharsets.UTF_8));
        }

        private void sendBinaryResponse(HttpExchange exchange, int statusCode, byte[] data) throws IOException {
