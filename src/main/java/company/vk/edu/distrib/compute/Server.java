...
import company.vk.edu.distrib.compute.dariaprindina.DPKvClusterFactory;
import company.vk.edu.distrib.compute.dariaprindina.DPKvServiceFactory;
...
public class Server {
    ...
    void main(String... args) throws IOException {
        ...
        if (isClusterMode(args)) {
            ...
            KVCluster cluster = new DPKvClusterFactory().create(ports);
            cluster.start();
            log.info("Cluster started on ports={}", ports);
            Runtime.getRuntime().addShutdownHook(new Thread(cluster::stop));
        } else {
            var port = 8080;
            KVService storage = new DPKvServiceFactory().create(port);
            storage.start();
