...
/**
 * HTTP-сервер с кластером файловых узлов и кворумной репликацией.
 *
 * <p>Архитектура:
 * <ul>
...
public class KVServiceReplicaImpl implements KVService {
    ...
    private void handleEntity(HttpExchange exchange) throws IOException {
        try (exchange) {
            try {
                ...
                int ack = parseAck(params, exchange);
                if (ack < 0) {
                    return;
                }
                ...
                switch (exchange.getRequestMethod()) {
                    case METHOD_GET -> handleGet(exchange, id, ack, replicaIndices);
                    case METHOD_PUT -> handlePut(exchange, id, ack, replicaIndices);
                    case METHOD_DELETE -> handleDelete(exchange, id, ack, replicaIndices);
     ...
     * @param exchange HTTP-обмен для отправки 400 при ошибке валидации
     * @return значение ack (>= 1) или -1 если ответ 400 уже отправлен
     */
    private int parseAck(Map<String, String> params, HttpExchange exchange) throws IOException {
